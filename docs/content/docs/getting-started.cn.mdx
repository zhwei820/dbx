...
<Steps>
  ...
  <Step>
    ...
    同一个输入框也接受 mysql 系客户端命令，例如 `mycli -hlocalhost -P32804 -uroot -p123456 tms` 或 `mysql -h db -P 3306 -u root -psecret shop`，可以直接粘贴终端里现成的命令。

    对已有的 MySQL 连接，也可以在侧边栏右键该连接选择「替换连接信息」，粘贴同样格式的命令即可替换主机、端口、用户名、密码和数据库并重新连接；连接名称、驱动、SSH 隧道等设置保持不变。
