...
<Steps>
  ...
  <Step>
    ...
    The same field also accepts a mysql-family client command, such as `mycli -hlocalhost -P32804 -uroot -p123456 tms` or `mysql -h db -P 3306 -u root -psecret shop`, so a command you already have in a terminal can be pasted as is.

    For a MySQL connection that already exists, right-click it in the sidebar and choose **Replace Connection Info** to paste a command in the same format: host, port, username, password and database are replaced and the connection reopens, while its name, driver, SSH tunnel and other settings stay as they are.
